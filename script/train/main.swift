...
enum Defaults {
    ...
    /// 39.8 万件 × 9 epoch: 未学習かな CER 3 層 15.90% → 4 層 14.11%、RTF 0.0090 → 0.0103。
    /// 短い学習 (6 万件 × 2 epoch) では 4 層の方が悪く見えるので、層数は収束まで回して比べる。
    /// 4 層は lrMin に入っても held-out が下がり続けたので、同じ理屈で 5 層を試す
    static let numLayers = 5
